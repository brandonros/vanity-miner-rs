...
register_self_test! {
    ...
    fn dalek_scalar_round_trip_one() -> u32 {
...
register_self_test! {
    ...
    fn dalek_mul_base_scalar_one() -> u32 {
...
register_self_test! {
    ...
    fn dalek_scalar52_montgomery_reduce_r() -> u32 {
...
register_self_test! {
    /// dalek Scalar52 mul(1, R) then reduce without sub == 1
...
register_self_test! {
    ...
    fn dalek_scalar52_montgomery_reduce_with_sub() -> u32 {
        let r = bisect_scalar52::R;
        let mut widened = [0u128; 9];
        for (i, x) in r.0.iter().enumerate() {
            widened[i] = *x as u128;
...
register_self_test! {
    /// dalek canonical and modular order boundaries
    fn dalek_order_boundaries() -> u32 {
        use curve25519_dalek::Scalar;
        let order = super::scalar_fixtures::SCALAR_ORDER;
        let mut below = order;
        below[0] -= 1;
        let mut above = order;
        above[0] += 1;
        let parsed: Option<Scalar> = Scalar::from_canonical_bytes(black_box(below)).into();
        let Some(parsed) = parsed else {
            return 0;
        };
        u32::from(
            parsed.to_bytes() == below
                && bool::from(Scalar::from_canonical_bytes(black_box(order)).is_none())
                && bool::from(Scalar::from_canonical_bytes(black_box(above)).is_none())
                && Scalar::from_bytes_mod_order(black_box(order)).to_bytes() == [0; 32]
                && Scalar::from_bytes_mod_order(black_box(above)).to_bytes() == Scalar::ONE.to_bytes(),
        )
    }
}

register_self_test! {
    /// dalek nonzero wide reduction known answer
    fn dalek_wide_nonzero() -> u32 {
        let input = black_box(core::array::from_fn::<_, 64, _>(|i| i as u8));
        u32::from(
            curve25519_dalek::Scalar::from_bytes_mod_order_wide(&input).to_bytes()
                == super::scalar_fixtures::WIDE_REDUCED,
        )
    }
}
