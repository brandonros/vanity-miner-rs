...
use crate::crypto::secp256k1::try_secp256k1_derive_public_key;
...
use crate::modes::bitcoin::try_generate_and_check_bitcoin_vanity_key;
...
use crate::test_vectors::SECP256K1_PRIMITIVE_COMPRESSED_PUB;
...
register_self_test! {
    ...
    fn primitive_secp256k1_compressed() -> u32 {
        let pub_key = try_secp256k1_derive_public_key(&crate::self_test::black_box(SECP256K1_PRIMITIVE_PRIV));
        (pub_key == Some(SECP256K1_PRIMITIVE_COMPRESSED_PUB)) as u32
    }
}
...
register_self_test! {
    ...
    fn primitive_ripemd160() -> u32 {
        let hash = ripemd160_32bytes_from_bytes(&crate::self_test::black_box(HASH_PRIMITIVE_INPUT_32));
...
register_self_test! {
    ...
    fn primitive_sha256_32() -> u32 {
        let hash = sha256_32_from_bytes(&crate::self_test::black_box(HASH_PRIMITIVE_INPUT_32));
...
fn bitcoin_test() -> Option<BitcoinVanityKeyResult> {
    let prefix = crate::self_test::black_box(*b"bc1q");
    let suffix = crate::self_test::black_box(*b"");
    let req = BitcoinVanityKeyRequest {
        prefix: &prefix,
        suffix: &suffix,
        thread_idx: crate::self_test::black_box(1),
        rng_seed: crate::self_test::black_box(13278869120712471092),
    };
    try_generate_and_check_bitcoin_vanity_key(&req)
}
...
register_self_test! {
    ...
    fn private_key() -> u32 {
        ...
        u32::from(bitcoin_test().is_some_and(|result| result.private_key == expected))
    }
}
...
register_self_test! {
    ...
    fn public_key() -> u32 {
        ...
        u32::from(bitcoin_test().is_some_and(|result| result.public_key == expected))
    }
}
...
register_self_test! {
    ...
    fn pkh() -> u32 {
        ...
        u32::from(bitcoin_test().is_some_and(|result| result.public_key_hash == expected))
    }
}
...
register_self_test! {
    ...
    fn encoded() -> u32 {
        ...
        let Some(btc) = bitcoin_test() else { return 0; };
...
register_self_test! {
    /// bitcoin matches
    fn matches() -> u32 {
        u32::from(bitcoin_test().is_some_and(|result| result.matches))
    }
}
...
use crate::test_vectors::BECH32_P2WPKH_HASH;

use crate::test_vectors::BECH32_P2WPKH_EXPECTED;
