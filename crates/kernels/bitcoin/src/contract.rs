...
unsafe impl Contract for Bitcoin {
    ...
    fn candidate(
        ...
        let _ = payload;
        logic::modes::bitcoin::candidate(request, counter, pattern)
    }
}
...
mod tests {
    ...
    fn layout_matching_and_invalid_requests() {
        ...
        let mut pattern = Pattern::new(b"bc1q", b"6m").unwrap();
        let result = Bitcoin::candidate(&seed, &pattern, &[], 0);
        ...
        pattern.suffix[0] ^= 1;
        assert_eq!(
            Bitcoin::candidate(&seed, &pattern, &[], 0).status,
