//! Structured candidate entry for solana.
...
#[unsafe(no_mangle)]
pub unsafe extern "ptx-kernel" fn kernel_solana_vanity(
    ...
    let result = match start.checked_add(lane as u64) {
        Some(counter) => unsafe { logic::modes::solana::candidate(&*request, counter, &*pattern) },
