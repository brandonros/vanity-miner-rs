//! Original rsa-pss known-answer checks through stock Rust and direct Metal.
#![no_std]
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn kernel_self_test_rsa_pss(selector: *const u32, results: *mut u32) {
    let slot = unsafe { selector.read() };
    let results =
        unsafe { core::slice::from_raw_parts_mut(results, logic::self_test::SELF_TEST_NUM_CHECKS) };
    logic::self_test::runners::rsa_pss::run_slot(results, slot as usize);
}
...
mod tests {
    ...
    fn original_checks_and_slot_selection_preserve_guards() {
        ...
        let owner = "kernel_self_test_rsa_pss";
        for selector in [
            u32::MAX,
            ...
            unsafe { kernel_self_test_rsa_pss(&selector, results.as_mut_ptr().add(1)) };
    ...
    fn exported_cases_execute_original_checks_and_only_their_slot() {
        ...
        {
            let entry = logic::self_test::runners::rsa_pss::metal_entry(case.slot).unwrap();
