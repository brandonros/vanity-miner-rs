...
fn check_runner(device: bool) {
    ...
    for (i, source) in sources.into_iter().enumerate() {
        ...
        if device {
            ...
            crate::modes::tests::assert_continuous(if i == 3 { 1 } else { 2 }, |control| {
        ...
        let signature: [u8; 256] = crate::modes::tests::console_field(record, "signature")
            .try_into()
            .unwrap();
        let salt = crate::modes::tests::console_field(record, "salt");
        let winning_message = crate::modes::tests::console_field(record, "message");
...
fn evaluate_batch(
    request: &logic::modes::rsa_pss::RsaPssRequest,
    pattern: &HexPattern,
    message: &[u8],
    start: u64,
    count: u32,
