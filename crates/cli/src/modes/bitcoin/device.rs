...
pub const ENTRY: &str = "kernel_bitcoin_vanity";
...
fn verify(
    ...
    let result = logic::modes::bitcoin::generate_and_check_bitcoin_vanity_key(
        &logic::modes::bitcoin::BitcoinVanityKeyRequest {
            prefix,
            suffix,
            rng_seed,
            thread_idx,
    ...
    if !result.matches || bytes[..32] != result.private_key || bytes[32..].iter().any(|&b| b != 0) {
        return Err("device bitcoin candidate failed CPU verification".into());
    }
    let address = std::str::from_utf8(&result.encoded_public_key[..result.encoded_len])
        .map_err(|e| e.to_string())?;
    let mut wallet = zeroize::Zeroizing::new([0; 64]);
    let length =
        logic::modes::bitcoin::private_key_to_wif(&result.private_key, true, false, &mut wallet);
    let wallet = std::str::from_utf8(&wallet[..length]).map_err(|e| e.to_string())?;
    Ok(format!(
        "[bitcoin] private_key={}\n[bitcoin] public_key={}\n[bitcoin] hash160={}\n[bitcoin] address={}\n[bitcoin] wallet={}",
...
mod tests {
    ...
    fn verifies_candidate_and_rejects_corrupted_payload() {
        ...
        let result = logic::modes::bitcoin::candidate(&seed, 33, &pattern);
