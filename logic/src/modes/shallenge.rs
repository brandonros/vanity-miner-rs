...
mod tests {
    ...
    #[test]
    fn test_dynamic_username_lengths() {
        for (username, nonce_len) in [
            (&b"a"[..], 30),
            (&b"brandon"[..], 24),
            (&b"abcdefghijklmnopqrstuvwxyz1234"[..], 1),
        ] {
            let target = [0xff; 32];
            ...
            assert_eq!(result.nonce_len, nonce_len);
            let mut preimage = username.to_vec();
            preimage.push(b'/');
