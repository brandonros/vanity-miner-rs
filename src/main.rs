...
fn device_main(ordinal: usize, vanity_prefix: String, blocks_per_grid: usize, threads_per_block: usize) -> Result<(), DriverError> {
    ...
    loop {
        ...
        if found_flag[0] != 0.0 {
            ...
            println!("[{ordinal}] Found {matches_found} matches in {elapsed:?} ({matches_per_second:.2} matches/sec, {operations_per_second:.2}M ops/sec) with {attempts} attempts blocks_per_grid = {blocks_per_grid} threads_per_block = {threads_per_block}");
        }
    }
}
