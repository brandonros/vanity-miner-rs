...
pub struct GlobalStats {
    ...
    details: std::sync::OnceLock<Box<dyn Fn(Duration) -> String + Send + Sync>>,
}
...
impl GlobalStats {
    ...
    pub fn print_progress(&self) {
        ...
        if let Some(render) = self.details.get() {
            let details = render(elapsed);
            if !details.is_empty() {
                println!("{details}");
            }
        }
    ...
    /// Attach the active mode's progress display once, before workers start.
    pub fn set_details(
        &self,
        render: impl Fn(Duration) -> String + Send + Sync + 'static,
    ) -> Result<(), String> {
        self.details
            .set(Box::new(render))
            .map_err(|_| "progress details already configured".into())
    }
...
pub fn print_verified(
    ...
    writeln!(std::io::stdout().lock(), "{}", record.as_str()).map_err(|e| e.to_string())?;
