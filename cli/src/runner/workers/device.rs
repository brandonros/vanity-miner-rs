...
pub fn run<State>(
    ...
    if continuous && control.continuous_device() {
        return run_independent(count, &control, initialize, evaluate);
    }
...
/// Each device owns its search for its entire lifetime. Finite completion lets
/// peers drain assigned work; errors and panics cancel all devices.
fn run_independent<State>(
    count: usize,
...
mod tests {
    ...
    #[test]
    fn continuous_devices_finish_independently_without_winner_rounds() {
