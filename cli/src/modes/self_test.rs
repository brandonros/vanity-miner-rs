...
pub mod gpu {
    ...
    pub fn run(
        ...
        let mut stub = [0u32; logic::SELF_TEST_NUM_CHECKS];
        let stub_dev = DeviceBuffer::<u32>::zeroed(logic::SELF_TEST_NUM_CHECKS)?;
        let stub_kernel = module.get_function("kernel_self_test_stub")?;
        unsafe {
            launch!(
                stub_kernel<<<1u32, 1u32, 0, stream>>>(
                    stub_dev.as_device_ptr(),
                )
            )?;
        }
        ...
        macro_rules! run_slot {
            ($slot:expr, $kname:expr) => {{
                ...
                let kernel = match module.get_function(kname) {
                    Ok(k) => k,
                    Err(e) => {
                        return Err(format!(
                            "[{ordinal}] slot {slot:2} [{label}] get_function({kname}) failed: {e}"
                        )
                        .into());
                    }
                };
                let launch_res = unsafe {
                    launch!(
                        kernel<<<1u32, 1u32, 0, stream>>>(
                            results_dev.as_device_ptr(),
                        )
                    )
                };
