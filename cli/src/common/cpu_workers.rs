...
/// Signals peers on normal return, error, or panic unwinding.
/// With panic=abort (the CLI profiles), a panic terminates the whole process.
...
where
    T: Send + Sync + 'static,
    F: Fn(usize, Arc<T>, Arc<AtomicBool>) -> WorkerResult + Send + Clone + 'static,
{
    ...
    for i in 0..num_workers {
        ...
        match std::thread::Builder::new().spawn(move || {
            let _guard = CancelOnExit(stop.clone());
            f(i, data, stop)
        }) {
