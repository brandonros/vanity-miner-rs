...
pub fn encode_into_limbs(input: &[u8], output: &mut [u8]) -> usize {
    ...
    for idx in (0..index).rev() {
        ...
        for i in 0..DIGITS_PER_LIMB {
            let temp = limb_value / DIVISORS[i]; // TODO: replace me with a div table lookup
            let temp = temp % 58; // TODO: replace me with a mod table lookup
