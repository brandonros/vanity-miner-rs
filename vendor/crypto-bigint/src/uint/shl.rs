...
impl<const LIMBS: usize> Uint<LIMBS> {
    ...
    pub const fn shl_vartime(&self, n: usize) -> Self {
        ...
        // Walk the source forward so LLVM can prove both array bounds before
        // lowering to targets without a Rust panic runtime. Same limb copies;
        // the trip count still depends only on the public shift distance.
        let mut source = 0;
        while source < LIMBS - shift_num {
            limbs[source + shift_num] = self.limbs[source];
            source += 1;
        }
