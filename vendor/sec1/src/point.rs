    ...
    Size: ModulusSize,
{
    ...
    /// Get the serialized bytes, returning an error for an invalid SEC1 tag.
    ///
    /// This checked accessor is useful for device builds that cannot support
    /// the panic path in [`Self::as_bytes`]. Valid points have identical bytes.
    pub fn try_as_bytes(&self) -> Result<&[u8]> {
        let tag = Tag::from_u8(self.bytes[0])?;
        self.bytes
            .get(..tag.message_len(Size::to_usize()))
            .ok_or(Error::PointEncoding)
    }
...
mod tests {
    ...
    #[test]
    fn checked_bytes_match_all_valid_tags_and_reject_invalid_tags() {
        for byte in 0u8..=u8::MAX {
            let mut point = EncodedPoint::identity();
            point.bytes[0] = byte;
            match Tag::from_u8(byte) {
                Ok(tag) => {
                    let bytes = point.try_as_bytes().unwrap();
                    assert_eq!(bytes, point.as_bytes());
                    assert_eq!(bytes.len(), tag.message_len(32));
                }
                Err(_) => assert!(point.try_as_bytes().is_err()),
            }
        }
    }
