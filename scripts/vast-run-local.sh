...
PORT="${VAST_PORT:-34749}"
HOST="${VAST_HOST:-ssh8.vast.ai}"
USER=root

LOCAL_BINARY="${LOCAL_BINARY:-target/release/vanity-miner}"
LOCAL_PTX="${LOCAL_PTX:-target/ptx/kernels.ptx}"
if [ ! -f "$LOCAL_PTX" ]; then
    echo "ERROR: PTX missing: $LOCAL_PTX. Run scripts/build-gpu.sh first."
    exit 1
fi

if [ ! -f "$LOCAL_BINARY" ]; then
    echo "ERROR: $LOCAL_BINARY not found. Build it first with: bash scripts/build-gpu.sh"
    exit 1
fi
...
scp -o StrictHostKeyChecking=no -P $PORT "$LOCAL_PTX" $USER@$HOST:kernels.ptx
